...
if (isset($_POST['curator_track'])) 
{
		...
		<script type="text/javascript"> 
			...
			SC.get('/resolve', { url: songUrl }, function(track)
			{
				...
				else if (!streamable) 
				{
					if(source = 'dropPage')
					{
						window.location.href = "../view/curator_index.php?message=notStreamable";
					}
					else
					{
				...
				else
				{
					if(source = 'dropPage')
					{
						window.location.href = "../view/curator_index.php?message=notWorking";
					}
					else
					{
	...
	{
		$curatorId = $_SESSION['id_curator'];
		include_once('mailYoutubeLink.php');
	}
	else
	{
		$curatorId = $_SESSION['id_curator'];
		echo('<script>window.location.href = "../view/curator_index.php?message=badLink&curatorId='.$curatorId.'";</script>');
	}
}
