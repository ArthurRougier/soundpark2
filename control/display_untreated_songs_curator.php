...
include_once("../model/get_curators_week_songs.php");				
	...
	echo '<form <form accept-charset="UTF-8" action="../control/modify_curators_songs2.php" id="update_untreated_songs_curator" method="post">';
		...
		{				
				...
				echo('   <a  href="../control/remove_week_track_curator.php?idSong='.$trackList[0].'&curatorId='.$_GET['curatorId'].'">Remove</a> ');
				echo ('-');
				echo('  <a  target=\"_blank\" href="'.$trackList[9].'">Link</a></br><input autofocus="autofocus" class="songId" id="songId'.$index.'" name="songId'.$index.'" value="'.$trackList[0].'" type="hidden"/><input autofocus="autofocus" class="trackId" id="trackId'.$index.'" name="trackId'.$index.'" value="'.$trackList[8].'" type="hidden"/></li>');
				$index++;
